...
class Configuration:
    ...
    def __init__(self):
        ...
        # --------- RUN NAME ---------
        self.run_name = 'MACS_reprocessing_1024p'                     # custom name for this run, eg resampled_x3, alpha60, new_train etc

        # ---------- PATHS -----------/mnt/raid1
        # Path to training areas and polygons shapefiles
        self.training_data_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/training'
        self.training_area_fn = 'aoi_utm_8a.gpkg'#'merged_rectangles.gpkg' #'rectangles_planet.gpkg'
        self.training_polygon_fn = 'dw_utm_8a.gpkg' #'merged_polygons.gpkg' #'polygons_planet.gpkg'
        
        # Path to training images
        self.training_image_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/training_images' #'/nfs/Planet/PlanetScope/Mosaics/Africa/2019' 

        # Output base path where all preprocessed data folders will be created
        self.preprocessed_base_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/training_data'
        ...
        self.preprocessed_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/training_data/20240816-0921_MACS_reprocessing_1024p/real'               # if set to None, it will use the most recent preprocessing data

        # Path to existing model to be used to continue training on [optional]
        self.continue_model_path = None # r"/isipd/projects/p_planetdw/data/dw_detection/aerial/models/20241004-0854_MACS_reprocessing_1024p.h5" #'\mnt\disk3\dw_data\driftwood\models\Planet\20230914-0848_Planet_dwmacs256_nonorm.h5'             # If set to None, a new model is created

        self.evaluate_model_path = '/isipd/projects/p_planetdw/data/dw_detection/aerial/models/20240126-2220_Planet_dwplanet256_macs.h5'
        self.evaluation_path = '/isipd/projects/p_planetdw/data/dw_detection/aerial/evaluation'

        
        # Path where trained models and training logs will be stored
        self.saved_models_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/models'
        self.logs_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/logs'
       

        # Paths to input images to be predicted
        self.to_predict_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/images' #'\nfs\Planet\PlanetScope\Mosaics\South_Asia\2021'
        ...
        self.predictions_base_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/results' # '\nfs\Users\Martin\RE\results_planet'

        # Path to model to use for this prediction [optional]
        self.trained_model_path = r"/isipd/projects/p_planetdw/data/dw_detection/aerial/models/20241009-0833_MACS_reprocessing_1024p.h5"         # if set to None, it will use the most recent trained model

        # Path to prediction folder to be postprocessed
        self.postprocessing_dir = r'/isipd/projects/p_planetdw/data/dw_detection/aerial/results'             # if set to None, it will run postprocessing on the last predictions
        ...
        self.prediction_batch_size = 50           # Depends on GPU memory, patch size and num parallel workers
        self.prediction_stride = 512               # stride = width -> no overlap, stride = width/2 -> 50 % overlap

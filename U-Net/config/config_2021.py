...
class Configuration:
    ...
    def __init__(self):
        ...
        # --------- RUN NAME ---------
        self.run_name = '2021'                     # custom name for this run, eg resampled_x3, alpha60, final_run_final_final
        # ---------- PATHS -----------
        ...
        self.training_area_fn = 'utm_3/aoi.gpkg'#'merged_rectangles.gpkg' #'rectangles_planet.gpkg'
        self.training_polygon_fn = 'utm_3/dw.gpkg' #'merged_polygons.gpkg' #'polygons_planet.gpkg'

        
        # Path to training images
        self.training_image_dir = '/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/images/2019_dem/' #'/nfs/Planet/PlanetScope/Mosaics/Africa/2019' 

        # Output base path where all preprocessed data folders will be created
        self.preprocessed_base_dir = '/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/training_data/temp/'
        ...
        self.preprocessed_dir = "/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/training_data/training_data_v4_dem"               # if set to None, it will use the most recent preprocessing data

        # Path to existing model to be used to continue training on [optional]
        self.continue_model_path =  None #"/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/models/20241004-0744_RGBN+NDVI+DEM+2atrous.h5" #'/mnt/disk3/dw_data/driftwood/models/Planet/20230914-0848_Planet_dwmacs256_nonorm.h5'             # If set to None, a new model is created

        self.evaluate_model_path = None

        # Path where trained models and training logs will be stored
        self.saved_models_dir = '/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/models/'
        self.logs_dir = '/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/logs/'
       

        # Paths to input images to be predicted
        self.to_predict_dir = '/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/images/2021_dem' #'/nfs/Planet/PlanetScope/Mosaics/South_Asia/2021'
        ...
        self.predictions_base_dir = '/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/results' # '/nfs/Users/Martin/RE/results_planet'
        ...
        self.postprocessing_dir = '/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/results/valid_obs'              # if set to None, it will run postprocessing on the last predictions
        ...
        # ----- PREDICTION CONFIG ------
        self.prediction_workers = 4                # number of prediction processes predicting image chunks in parallel
        self.prediction_gridsize = (50, 30)          # num rows/cols in which images are split for parallel processing
        self.prediction_batch_size = 2           # Depends on GPU memory, patch size and num parallel workers
...
# =============================================================================
        ...
        self.prediction_output_dir = '/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/results/2021' #'/isipd/projects/p_planetdw/data/dw_detection/PlanetScope/results/20241021-0711_dem_only/'        # set dynamically at prediction time
